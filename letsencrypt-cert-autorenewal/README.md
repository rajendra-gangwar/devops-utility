...
- Custom templates support variable substitution (see Template Customization below)
- **Notification title/subject format:** `<STATUS>: Let's Encrypt Certificate Auto Renewal Notification for <certificate_name>`
- **Expiry date in notifications:**
  - **SUCCESS**: Shows the **new certificate expiry date** (parsed from the renewed certificate)
  - **FAILURE**: Shows the **old certificate expiry date** (still relevant since renewal failed)
...
- Expiry date (new or old based on status)
...
You can customize notification templates by providing your own HTML (email) or JSON (Teams) files.
...
| `{{san_list}}` | Subject Alternative Names (comma-separated) |
| `{{expiry_date}}` | Certificate expiry date (new for SUCCESS, old for FAILURE) |
| `{{status}}` | Renewal status: `SUCCESS` or `FAILED` |
| `{{status_emoji}}` | Status emoji: ✅ (success) or ❌ (failure) |
| `{{failure_reason}}` | Error message if renewal failed (empty for success) |

**Email-only placeholders** (for styling based on status):

| Placeholder | SUCCESS Value | FAILED Value |
|-------------|---------------|--------------|
| `{{header_bg_color}}` | `#28a745` (green) | `#dc3545` (red) |
| `{{status_bg_color}}` | `#d4edda` (light green) | `#f8d7da` (light red) |
| `{{status_text_color}}` | `#155724` (dark green) | `#721c24` (dark red) |
| `{{border_color}}` | `#28a745` (green) | `#dc3545` (red) |
| `{{failure_section}}` | Empty | HTML block with failure details |

**Email template notes:**
- Use **inline styles** for maximum email client compatibility
- Use **table-based layouts** instead of CSS flexbox/grid
- Avoid CSS gradients and hover effects (not supported in most email clients)
- The default template is optimized for Gmail, Outlook, and other major email clients

**Teams template notes:**
- Must be a valid [Adaptive Card](https://adaptivecards.io/) JSON format
- The `failure_reason` field shows "N/A" for successful renewals
...
| `TEAMS_WEBHOOK_URL` | For Teams | Microsoft Teams incoming webhook URL (required if Teams notifications enabled) |
...
- Verify `TEAMS_WEBHOOK_URL` environment variable is set (this is the only way to configure the webhook URL)
- Check that `teams.enabled` is `true` in config
- Verify the webhook URL is correct and active
- Teams webhooks only work with public channels (not private channels)
- Teams webhook URLs expire if the connector is removed from the channel

**Notifications sent but content is wrong**
- If using custom templates, verify variable names match the available placeholders (see Template Customization section)
- Check template file path is correct relative to the project root
- For email templates, ensure you're using inline styles (external CSS is not supported in email clients)

**Expiry date shows old date after successful renewal**
- This should not happen - successful renewals show the new expiry date parsed from the renewed certificate
- If this occurs, check that the certificate file was successfully generated by Certbot
- Verify the `cryptography` library is installed (`pip install cryptography`)
